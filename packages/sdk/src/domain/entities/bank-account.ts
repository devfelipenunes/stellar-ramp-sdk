/**
 * Per-country bank account details — map to the Etherfuse `account` object
 * (oneOf `AccountRegistration`). Confirmed in the docs 03/08/2026:
 *
 *   BRL/PIX individual:  transactionId, firstName, lastName, cpf, pixKey, pixKeyType
 *   BRL/PIX business: transactionId, name, cnpj, pixKey, pixKeyType
 *   MXN/SPEI individual:  transactionId, firstName, paternalLastName,
 *                         maternalLastName, birthDate (YYYYMMDD),
 *                         birthCountryIsoCode, curp (18), rfc (13), clabe (18)
 *
 * `kind` is our discriminator; the adapter builds the flattened object without
 * sending it (Etherfuse detects the variant from the fields). `transactionId`
 * is an idempotency key generated by the adapter (not returned by the API).
 * There is no bank/branch/account code — `pixKey`+`pixKeyType` is the destination.
 *
 * TODO(sandbox): MXN/SPEI business variant (shape not confirmed).
 */
...
/**
 * Runtime validation of per-country fields (fail-fast before the fetch).
 * The TS type covers shape at compile time; here we cover the content that
 * Etherfuse sandbox/prod rejects with an opaque error (wrong CLABE length,
 * broken RFC, empty pixKey...). Returns a list of problems; empty = ok.
 */
